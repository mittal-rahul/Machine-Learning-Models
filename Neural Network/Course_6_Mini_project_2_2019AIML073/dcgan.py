...
class DCGAN():
    ...
    def train(self, epochs, train_data, batch_size):
        ...
        real = np.ones((batch_size,1), dtype=int)
        ### Define fake as an array with values 0 of shape (batch_size, 1)
        fake = np.zeros((batch_size,1), dtype=int)
        history = []
        ...
        self._image_helper.makegif("generated_images/")        
    ...
    def _build_generator_model(self):
        ...
        generator_seqence = Sequential(
                [Dense(128 * 7 * 7, activation="selu", input_dim=self.generator_input_dim),
                 Reshape((7, 7, 128)),
                 UpSampling2D(),
                 ...
                 BatchNormalization(momentum=0.8),
                 Activation("selu"),
                 Conv2D(self.channels, kernel_size=5, padding="same"),
                 Activation("tanh")])
    ...
    def _build_and_compile_discriminator_model(self, optimizer):
        ...
        discriminator_sequence = Sequential(
                [Conv2D(32, kernel_size=5, strides=2, input_shape=self.img_shape, padding="same"),
                 LeakyReLU(alpha=0.2),
                 Dropout(0.25),
        ...
        ### compile discriminator model with binaryCE loss, Adam optimizer with learning rate of your choice, and accuracy as metric
    ...
    def _build_and_compile_gan(self, optimizer):
        ...
        ### compile gan model with binaryCE loss, Adam optimizer with learning rate of your choice (should be same as above)
        self.gan.compile(loss='binary_crossentropy', optimizer=optimizer)
    
    def _save_images(self, epoch):
        generated = self._predict_noise(25)
        generated = 0.5 * generated + 0.5
        self._image_helper.save_image(generated, epoch, "generated_images/")
